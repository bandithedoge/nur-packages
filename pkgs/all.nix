...
let
  allPackages = {
    ...
    python3Packages = callPackages' ./python3Packages { };
  ...
  callPackage' =
    ...
    pkgs.lib.callPackageWith (
      (pkgs.lib.recursiveUpdate pkgs allPackages)
      // {
        inherit callPackage';
  ...
  callPackages' =
    pkg: args: pkgs.lib.recurseIntoAttrs (pkgs.lib.makeExtensible (_: callPackage' pkg args));
in
allPackages
