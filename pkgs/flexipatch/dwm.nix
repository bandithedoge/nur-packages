...
pkgs.stdenv.mkDerivation rec {
  ...
  '' + (
    let patchesFile =
      if pkgs.lib.isDerivation conf || builtins.isPath patches then patches else pkgs.writeText "config.def.h" patches;
    in
  ...
  postPatch =
    let configFile =
      if pkgs.lib.isDerivation conf || builtins.isPath conf then conf else pkgs.writeText "config.def.h" conf;
    in
